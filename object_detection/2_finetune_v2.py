...
from custom_data2 import CustomDataset
...
def vae(
    ...
    for i in range(variant['max_iters']):
        ...
        for fold, (train_idx, test_idx) in enumerate(kf.split(data_train)):
            train_subset = Subset(data_train, train_idx)    
            val_subset = Subset(data_train, test_idx)
            train_loader = DataLoader(train_subset, batch_size=variant['batch_size'], shuffle=True, num_workers=4,collate_fn=custom_collate_fn)
            eval_loader = DataLoader(val_subset, batch_size=1, shuffle=True, num_workers=4,collate_fn=custom_collate_fn)
