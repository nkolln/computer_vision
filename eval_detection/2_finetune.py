...
def vae(
    ...
    data_train = CustomDataset(lst_image[:split1],lst_tensor[:split1],lst_cat[:split1])
    train_loader = DataLoader(data_train, batch_size=variant['batch_size'], shuffle=True, num_workers=4,collate_fn=custom_collate_fn)
    data_eval = CustomDataset(lst_image[split1:split2],lst_tensor[split1:split2],lst_cat[split1:split2])
    eval_loader = DataLoader(data_eval, batch_size=1, shuffle=True, num_workers=4,collate_fn=custom_collate_fn)
    data_test = CustomDataset(lst_image[split2:],lst_tensor[split2:],lst_cat[split2:])
    ...
    for i in range(variant['max_iters']):
        ...
        for j,(images,anno,label) in enumerate(train_loader):
            #Manipulate images for training
            
            #Adjust brightness
            images = images + int(random.uniform(-80/255, 80/255))

            #Add noise
            noise = np.random.normal(0,40/255,images.shape)
            images = images + noise
            ...
            images = torch.tensor(images,device=device).float().permute(0,3,1,2)
